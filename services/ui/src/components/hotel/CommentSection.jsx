            ...
            {!user ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div>
                  ...
                  <input type="number" min="1" max="10" value={review.ratingOverall} onChange={e => setReview({...review, ratingOverall: Math.max(1, Math.min(10, parseInt(e.target.value) || 1))})} required />
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.25rem', color: 'var(--text-secondary)' }}>Cleanliness</label>
                    <input type="number" min="1" max="10" value={review.ratingCleanliness} onChange={e => setReview({...review, ratingCleanliness: Math.max(1, Math.min(10, parseInt(e.target.value) || 1))})} required />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.25rem', color: 'var(--text-secondary)' }}>Staff</label>
                    <input type="number" min="1" max="10" value={review.ratingStaff} onChange={e => setReview({...review, ratingStaff: Math.max(1, Math.min(10, parseInt(e.target.value) || 1))})} required />
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                  <div>
                    ...
                    <input type="number" min="1" max="10" value={review.ratingFacilities} onChange={e => setReview({...review, ratingFacilities: Math.max(1, Math.min(10, parseInt(e.target.value) || 1))})} required />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.25rem', color: 'var(--text-secondary)' }}>Location</label>
                    <input type="number" min="1" max="10" value={review.ratingLocation} onChange={e => setReview({...review, ratingLocation: Math.max(1, Math.min(10, parseInt(e.target.value) || 1))})} required />
                  </div>
                </div>

                <div>
                  <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.25rem', color: 'var(--text-secondary)' }}>Comfort (1-10)</label>
                  <input type="number" min="1" max="10" value={review.ratingComfort} onChange={e => setReview({...review, ratingComfort: Math.max(1, Math.min(10, parseInt(e.target.value) || 1))})} required />
                </div>

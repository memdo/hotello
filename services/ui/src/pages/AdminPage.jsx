          ...
          {allHotels.length > 1 ? (
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '0.5rem' }}>
              <select 
                ...
                style={{ 
                  fontSize: '2.5rem', fontWeight: 800, margin: 0, padding: 0,
              ...
              >
                {allHotels.map(h => (
                  <option key={h.id} value={h.id} style={{ fontSize: '1rem', color: '#000' }}>{h.name}</option>
                ))}
              </select>
            </div>
          ) : (

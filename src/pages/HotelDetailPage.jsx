...
export default function HotelDetailPage() {
  ...
  const [bookingState, setBookingState] = useState({ checkIn: '', checkOut: '', guests: 2, roomTypeId: '' });
  ...
  const handleBook = async (e) => {
    ...
    try {
        ...
        const res = await fetch('/api/v1/hotels/book', { 
            ...
            body: JSON.stringify({...bookingState, hotelId: id}) 
        });
            ...
            {bookingSuccess ? (
              ...
              <form onSubmit={handleBook} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  ...
                  <input type="date" value={bookingState.checkIn} onChange={e => setBookingState({...bookingState, checkIn: e.target.value})} required />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.9rem', marginBottom: '0.5rem', color: 'var(--text-secondary)' }}>Check Out</label>
                  <input type="date" value={bookingState.checkOut} onChange={e => setBookingState({...bookingState, checkOut: e.target.value})} required />
                </div>
                <div>
